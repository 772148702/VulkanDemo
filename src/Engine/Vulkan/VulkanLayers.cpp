...
{
    std::vector<VulkanLayerExtension> globalLayerExtension(1);
    EnumerateInstanceExtensionProperties(nullptr, globalLayerExtension[0]);
    
    std::vector<std::string> foundUniqueExtension;
    for(int32 i=0;i<globalLayerExtension[0].extensionProps.size();++i)
    {
        StringUtils::AddUnique(foundUniqueExtension,globalLayerExtension[0].extensionProps[i].extensionName);
    }
    ...
    for (int32 i = 0; i < globalLayerProperties.size(); ++i) 
    {
        ...
        layer.AddUniqueExtensionNames(foundUniqueExtension);
        StringUtils::AddUnique(foundUniqueExtension,globalLayerProperties[i].layerName);
        globalLayerExtension.push_back(layer);
    }
    ...
    for(const std::string& name: foundUniqueExtension)
    {
    ...
    for(int32 i=0;G_ValidationLayersInstance[i]!=nullptr;++i)
    {
        ...
        bool found = FindLayerExtensionInList(globalLayerExtension,currValidationLayer);
        if(found)
        {
    ...
    if(FindLayerExtensionInList(globalLayerExtension,VK_EXT_DEBUG_REPORT_EXTENSION_NAME))
    {
    ...
    {
        if(FindLayerExtensionInList(globalLayerExtension,extension))
        {
    ...
    {
        if(FindLayerExtensionInList(globalLayerExtension,G_InstanceExtensions[i]))
        {
...
void VulkanDevice::GetDeviceExtensionsAndLayers(std::vector<const char*>& outDeviceExtensions, std::vector<const char*>& outDeviceLayers, bool& bOutDebugMarkers)
{
    ...
    std:std::vector<VulkanLayerExtension> deviceLayerExtensions(count+1);
    ...
    for(int32 index=0;index<deviceLayerExtensions.size();++index)
    {
        ...
        {
            StringUtils::AddUnique(foundUniqueExtensions,deviceLayerExtensions[index].layerProps.layerName);
